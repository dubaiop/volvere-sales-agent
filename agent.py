"""Sales Agent — Claude first, NVIDIA fallback if credits run out."""

from config import CLAUDE_MODEL, ANTHROPIC_API_KEY, COMPANY_NAME, NVIDIA_API_KEY
...
def _call(system: str, messages: list, max_tokens: int = 4096, model: str = None) -> str:
    ...
    if NVIDIA_API_KEY:
        ...
        r = client.chat.completions.create(
            model=_NVIDIA_MODEL, messages=msgs, max_tokens=min(max_tokens, 2048), temperature=0.7
        )
